...
void Tester::testAngles(void) {
    ...
    for (U32 index = 0; index < FW_NUM_ARRAY_ELEMENTS(angles); index++) {
        ...
        ASSERT_CMD_RESPONSE(0, ArmDemoComponentBase::OPCODE_AA_CLAW_ANG, 10, Fw::CmdResponse::OK);
        ...
        // verify the command response was sent
        ASSERT_CMD_RESPONSE_SIZE(1);
        ASSERT_CMD_RESPONSE(0, ArmDemoComponentBase::OPCODE_AA_BASE_ANG, 10, Fw::CmdResponse::OK);
        ...
        // verify the command response was sent
        ASSERT_CMD_RESPONSE_SIZE(1);
        ASSERT_CMD_RESPONSE(0, ArmDemoComponentBase::OPCODE_AA_ARM_HEIGHT_ANG, 10, Fw::CmdResponse::OK);
        ...
        // verify the command response was sent
        ASSERT_CMD_RESPONSE_SIZE(1);
        ASSERT_CMD_RESPONSE(0, ArmDemoComponentBase::OPCODE_AA_ARM_LENGTH_ANG, 10, Fw::CmdResponse::OK);
    }
}
...
void Tester ::from_position_handler(const NATIVE_INT_TYPE portNum, F32 angle) {
...
void Tester ::connectPorts(void) {
    ...
    for (NATIVE_INT_TYPE i = 0; i < 4; ++i) {
        this->component.set_position_OutputPort(i, this->get_from_position(i));
    }
